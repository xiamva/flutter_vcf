...
class TokenRefreshInterceptor extends Interceptor {
  ...
  void onError(DioException err, ErrorInterceptorHandler handler) async {
    ...
    if (err.response?.statusCode == 401) {
      ...
      try {
        ...
        String tokenToRefresh = currentToken;
        if (tokenToRefresh.startsWith('Bearer ')) {
          tokenToRefresh = tokenToRefresh.substring(7);
        }

        print('[TokenRefresh] Attempting to refresh token for: ${requestOptions.path}');
        ...
        if (response.statusCode == 200) {
          ...
          if (data['success'] == true && data['data'] != null) {
            ...
            final bearerToken = 'Bearer $newToken';

            print('[TokenRefresh] Token refreshed successfully');

            // Save new token
            await prefs.setString('jwt_token', bearerToken);
            ...
            try {
              print('[TokenRefresh] Retrying original request: ${requestOptions.path}');
              final retryResponse = await _refreshDio.fetch(opts);
              _isRefreshing = false;
              
              // Resolve pending requests
              _resolvePendingRequests(bearerToken);
            ...
            print('[TokenRefresh] Refresh response invalid: ${data['message'] ?? 'Unknown error'}');
          }
        } else {
          print('[TokenRefresh] Refresh failed with status: ${response.statusCode}');
        }
  ...
  void _resolvePendingRequests(String newToken) {
    for (final pending in _pendingRequests) {
      ...
      _refreshDio.fetch(opts).then((response) {
